...
env_stan$prep_file_stan <- function(idtaskdep, indcode_list, train = TRUE,
  ...
  return(modifyList(result, list(
    ...
    data_stan_path = file.path(dir$stanmodel, paste0("data_stan_",control_code$out_prefix, "_", format(Sys.time(), '%y%m%d-%H%M%S'),".rds"))
  ))) 
}
...
env_stan$checkconverge_export <- function(draws_beta, vnames, out_prefix, dir_run,
  ...
  divergence_percent <- round(100 * colMeans(sapply(draws_beta$post_warmup_sampler_diagnostics, function(x) x$divergent__)), 1)
  ...
  if (makepdf){
    ...
    for (chain_i in (1:nchains)){
      matplot(1:nrow(draws_beta_mu[[chain_i]]), draws_beta_mu[[chain_i]],
              type = "l" , lty = 1, lwd = 1, main = paste0("Chain ", chain_i,  " Divergent = ", divergence_percent[i], "%"), xlab = "Iteration", ylab = "Mean Beta")   
    }
...
env_stan$eigen_priors_calc <- function(rate = 1.2, sigma = 1, mu_range = c(-5, 5)){
  # Sep 2026 compute mu so logit-normal(mu, sigma) has mean log(rate^2) 
  # given sigma and expected rate, compute mu of logitnormal (no closed form calc so iterate)
  lb <- mu_range[1]; ub <- mu_range[2]
  x_norm <- qnorm(seq(.01,.99, by = .0001), mean = 0, sd = sigma) # Perfectly distributed normal values
  mu_target <- 2 * log(rate) # Mean of our distribution should be log(rate^2), since eigen = sqrt(exp(x)))
  x_int <- 0
  iter <- 0
  range <- ub - lb
  while (range > .001){
    x_int <- mean(c(lb, ub))
    mu_logitnorm <- mean(1/(1+exp(-(x_norm + x_int))))
    if (mu_logitnorm > mu_target){
      ub <- x_int 
    }
    if (mu_logitnorm < mu_target){
      lb <- x_int
    ...
    iter <- iter + 1;
    range <- ub - lb;
    if (iter > 100) range <- 0 # Force convergence
  }
  if (abs(mu_logitnorm - mu_target) > .05){
    cat("Could not set logitnormal priors for eigenvalue based on your inputs\n")
    cat("Using defaults: rate = 1.2, sigma = 1 -> mu = -.66")
    lb <- ub <- -.66
    sigma <- 1
  }
  result <- c(mean(lb, ub), sigma) # mean to use so mean(logitnorm) = mu_target
  return(result)
}
...
env_stan$LC_est_v4 <- function(data_stan, data_model, out_folder, stanout, out_prefix, dir_save){
  if (min(data_model$LC_segs) > 0){
    ...
    LC_LL <- c()
    LC_pred <- cbind(row_in = data_stan$row_in, data_stan$idtask, holdout = data_stan$holdout[data_stan$idtask_r], wt = data_stan$wts[data_stan$idtask_r], dep = data_stan$dep)
...
env_stan$process_HB <- function(data_stan, data_model, control_code, meta_data,
  ...
  if (min(meta_data$return_codes) == 0){
    ...
    if (max(data_stan$task_scale_group > 1)) check_draws_vector(meta_data$output_files, "z_log_scale_xref",
                                                                paste0("z_log_scale_xref",1:(max(data_stan$task_scale_group)-1)),
                                                                control_code$dir_run, control_code$out_prefix,makepdf = ("scale_factor" %in% control_code$makepdf))
    ...
    if (control_code$export_draws_alphas) check_draws_vector(meta_data$output_files, "alpha", colnames(data_stan$ind), control_code$dir_run, control_code$out_prefix, makepdf = ("alpha" %in% control_code$makepdf))
