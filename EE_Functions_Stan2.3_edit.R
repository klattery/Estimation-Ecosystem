...
env_stan$prep_file_stan <- function(idtaskdep, indcode_list, train = TRUE,
  ...
  match_id <- match(idtask[, 1], resp_id)
  ...
  result$task_rowbeg <- result$start  # More descriptive name for start
  result$task_rowend <- result$end # More descriptive name for end
  result$task_individual <- match_id[result$start]
  list_id_tasks <- lapply(1:length(resp_id), function(x) which(x == result$task_individual))
  result$resp_begtask <- sapply(list_id_tasks, min)
  result$resp_endtask <- sapply(list_id_tasks, max)
